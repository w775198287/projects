...
My_Lua_log(" file=[ttgame] fun=[init];  tData.coins="..tData.coins)
    ...
    local NNGame = require("minigame.tt.tt.TTGame")
...
function OnApplicationDidEnterBackground(selfObj)
    ...
    My_Lua_log(" file=[ttgame] fun=[OnApplicationDidEnterBackground];")
end

function OnApplicationWillEnterForeground(selfObj)
    selfObj.nnGame:enterForeground()
    My_Lua_log(" file=[ttgame] fun=[OnApplicationWillEnterForeground];")
end
...
function OnClose(selfObj)
    ...
    My_Lua_log(" file=[ttgame] fun=[onclose]; remove   ok")
end
...
function OnExit(selfObj)
    ...
    My_Lua_log(" file=[ttgame] fun=[onexit];")
end

-- 刷新数据
function OnOpen(selfObj, miData)
    My_Lua_log(" file=[ttgame] fun=[OnOpen]; ")
...
function OnUpdate(selfObj, aiDelTime)
    if selfObj.miRoot:getChildrenCount() < 1 then
        ...
        aiEvent.miPagName = "ttgame"
        aiEvent.miData = {}
        selfObj.miEventMgr:Dispatch(aiEvent)
        My_Lua_log(" file=[ttgame] fun=[OnUpdate]; 111111111")
        --]]
