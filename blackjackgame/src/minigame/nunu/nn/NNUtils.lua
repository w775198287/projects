...
local NNUtils = class("NNUtils")
local randomseed = tonumber(os.time())
function NNUtils:init(target)
	self.nnGame_ = target
end
function NNUtils:Log( s )
	if NNGameData.isDebug then

		print(s)
	end
end
-- 为ImageView设置plist图片
function NNUtils:setPlistTexture(image, textureName)
	if image then
...
-- 为ImageView设置普通图片
function NNUtils:setLocalTexture(image, textureName)
	if image then
...
-- 为Text设置字符串
function NNUtils:setTextString(text, content, parent)
...
function NNUtils:findNodeByName(name, parent)
    if not parent then
		return
	end
...
-- 移除事件监听
function NNUtils:removeBackKeyEventListener(view)
...
-- 为view设置监听
function NNUtils:setOnViewClickedListener(view, listener, parent, effect, playSound,carom)
	if not carom then
		carom = 0.1
	end
...
-- 播放时间轴动画
function NNUtils:runTimelineAction(target, actionfile, loop, finishCallback)
...
-- 播放粒子效果
function NNUtils:runParticleEffect(parent, particlefile, x, y, blendSrc, blendDest)
...
-- 截取序列数组
function NNUtils:interceptSequenceTable(tb,length)
	if #tb <= length then
		return tb
	end
...
function NNUtils:downloadImage(url, callback)
	local imgType = ""
	...
	xhr._urlFileName = os.time().."_"..tostring(math.random(1,99999999)).."_"..tostring(math.random(1,99999999))--不能重名
	xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
	xhr:open("GET", url)
	local function onDownloadImage()
	    NNUtils:Log("xhr.readyState is:", xhr.readyState, "xhr.status is: ", xhr.status)
...
function NNUtils:showToast(content,dt)
	local winSize = cc.Director:getInstance():getWinSize()
	dt = dt or 1
	local toastNode = cc.CSLoader:createNode("nnToast.csb")
	...
	if self.nnGame_ then
		self.nnGame_:addChild(toastNode)
	end
end


function NNUtils:grayUIBtn(node,status)
...
function NNUtils:formtNumber( nNum, isSign)
	...
	if NNGameData.language==NNConstant.LANGUAGE_EN then
		return sign..string.format("%.2f", nNum/1000.0)..NNLanguage.a[13][NNGameData.language]
	elseif NNGameData.language==NNConstant.LANGUAGE_ZH_HANT then
		return sign..string.format("%.2f", nNum/10000.0)..NNLanguage.a[13][NNGameData.language]
	elseif NNGameData.language==NNConstant.LANGUAGE_ZH_SIMPLE then
		return sign..string.format("%.2f", nNum/10000.0)..NNLanguage.a[13][NNGameData.language]
	end
    return ""
end
function NNUtils:GetPhonePlatform()
...
--获取多语言资源位置
function NNUtils:getMutiRes(url)
	local newUrl = ""
...
--打乱数组
function NNUtils:shuffleTable( t )
	local len = #t
	for i=1,len do
...
function NNUtils:fileExists(filename)
	local file = io.open(filename, "rb")
	if file then file:close() end
	return file ~= nil
end
function NNUtils:lengthOfFile(filename)
...
function NNUtils:writeLog(filename, log)
	if not filename then
		filename = cc.FileUtils:getInstance():getWritablePath().."nnGameLog.txt"
	end
