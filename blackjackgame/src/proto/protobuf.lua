...
--[[
    local fact
    fact = function(selfmsg, msg)
        local fields = selfmsg

        for field, value in pairs(msg) do
            if field.label == LABEL_REPEATED and field.cpp_type ~= CPPTYPE_MESSAGE then
                selfmsg[field] = value
            elseif field.label == LABEL_REPEATED and field.cpp_type == CPPTYPE_MESSAGE then
                field_value = fields
                if field_value == nil then
                    field_value = field._default_constructor(selfmsg)
                    fields[field] = field_value
                end
                --self._fields[field]:MergeFrom(value)
                fact(selfmsg[field], field_value)
            else
                selfmsg[field] = value
            end
        end
    ...
    message_meta._member.MergeFrom = function (self, msg)
        ...
        for field, value in pairs(msg._fields) do
            ...
            elseif field.label == LABEL_REPEATED and field.cpp_type == CPPTYPE_MESSAGE then
                ...
                for field1, value1 in pairs(value) do
                    self._fields[field][field1] = value1
                end
                --fact(self._fields[field], field_value)
